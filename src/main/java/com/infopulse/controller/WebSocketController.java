...
public class WebSocketController extends TextWebSocketHandler {
    ...
    private Map<String, WebSocketSession> activeUsers = new ConcurrentHashMap<>();    //список активных пользователей

    private Validator validator;

    private ObjectMapper mapper = new ObjectMapper();

    public WebSocketController(UserService userService, WebSocketService webSocketService, Validator validator) {
    ...
    public void afterConnectionEstablished(WebSocketSession session) {
        try {
            ...
            if (user.isPresent()) {                                          //если пользователь зареган вообще
                if (user.get().getBan() != null) {                          //проверяем забанен ли
                    sendErrorMessage(session, "You are banned.");
                    return;
                }

                activeUsers.put(user.get().getLogin(), session);              //вносим пользователя в список активных пользователей
                sendActiveUsersList();                                       //обновляем список активных пользователей (уведомляем пользователей)
            } else {
                session.close();
            }
        } catch (IOException e) {
    ...
    @Override
    public void handleTextMessage(WebSocketSession session, TextMessage message) {  //если юзер не заваторизировался или забаненый закрываем сессию
        try {
            String login = (String) session.getAttributes().get("login");
            Optional<User> user = userService.findUserByLogin(login);
            ...
            ReceiveMessage receiveMessage = mapper.readValue(jsonString, ReceiveMessage.class);  //парсим джейсон с помощью джексона
    ...
    private void sendAll(TextMessage textMessage) {    // переписать дома через foreach
